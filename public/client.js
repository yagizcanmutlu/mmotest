...
(() => {
  ...
  // Gender radios HTML’de olmayabilir; default kalsın
  const genderRadios = document.querySelectorAll('input[name="gender"]');
  let selectedGender = 'female';
  genderRadios.forEach(r => r.addEventListener('change', e => { selectedGender = e.target.value; }));
  ...
  const dir  = new THREE.DirectionalLight(0x8fd0ff, 0.85);
  ...
  groundMat.polygonOffset = true; // z-fight ihtimallerine karşı
  ...
  const starPos = [];
  for (let i=0;i<3000;i++) starPos.push((Math.random()-0.5)*350, 30+Math.random()*90, (Math.random()-0.5)*350);
  ...
  try {
    ...
    console.warn('[Agora] GLTFLoader init hatası:', e);
  }

  // GLB’yi zemine oturt (pivot farklarını telafi et)
  function placeOnGround(root, targetY = 0){
  ...
  function buildPlayerFromGLB(name = "Player"){
    ...
    enableShadows(clone);
    placeOnGround(clone, 0);           // AYAKLAR zeminde
    holder.add(clone);
  ...
  function _createSpaceBaseDiscMesh(radius, opts = {}) {
    const params = { tilesPerUnit: 2.8, groove: 0.03, bevel: 0.015, stripeDensity: 7.0, emissiveK: 1.35, caution: new THREE.Color("#ffd166"), baseColor: 0x131a24, ...opts };
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        .replace('#include <common>', `
          ...
          float grooveMask(vec2 p){ vec2 gv = fract(p) - 0.5; vec2 dv = (0.5 - abs(gv)); float edge = min(dv.x, dv.y); return 1.0 - smoothstep(uGroove-uBevel, uGroove+uBevel, edge); }
          float cautionStripes(vec2 p){
            vec2 cell = floor(p); vec2 gv = fract(p) - 0.5; float rnd = hash21(cell);
            if (rnd < 0.66) return 0.0;
            float dens = uStripeDensity + floor(rnd*3.0); float dir = (rnd>0.82)? 1.0 : -1.0;
  ...
  // ---------- GLB MODELLERİ YÜKLE ----------
  if (gltfLoader){
    // Karakteri preload et ve local’i GLB’ye çevir
    gltfLoader.load('/models/readyplayermale_cyberpunk.glb', (gltf)=>{
      baseCharGLB = gltf.scene;
      // Henüz sahneye debug olarak EKLEME – sadece avatar swap
      swapLocalToGLB();
    }, undefined, (e)=>console.error('Karakter GLB yüklenemedi:', e));

    // Araba — zemine oturtup kenara koy
    gltfLoader.load('/models/cyberpunk_car.glb', (g)=>{
      const car = g.scene;
      car.scale.set(0.9, 0.9, 0.9);
      enableShadows(car);
      placeOnGround(car, 0);        // ← çakılma/üçgen sorununun kökü buydu
      car.position.set(6, car.position.y, 12);
  ...
  playBtn.addEventListener("click", () => {
    ...
    swapLocalToGLB(); // baseCharGLB geldiyse GLB’ye dön
  ...
  socket.on("bootstrap", ({ you, players, hotspots, planets }) => {
    ...
    if (you.gender === 'female' || you.gender === 'male') selectedGender = you.gender;

    swapLocalAvatar(selectedGender);
    swapLocalToGLB(); // baseCharGLB artık geldiyse GLB’ye dön
    ...
    // Burada kendini eklemeyi engelle
    (players || []).forEach(p => {
      if (p.id === local.id) return; // ← önemli fix
  ...
  socket.on("emote", ({ id, type, until }) => {
    ...
    if (!target) return;
    const p = document.createElement("p");
    p.innerHTML = `<i style="opacity:.8">[emote] ${(id===local.id?(local.name||'You'):(remotes.get(id)?.name||'Yogi'))}: /${type}</i>`;
  ...
  const planetUpdate = ()=>{ for (const p of planetMeshes) p.mesh.rotation.y -= 0.0012; };
  ...
  function tick(now){
    ...
    let forward = kForward + (isTouch ? 0 : 0) + (joyVec.y||0);
